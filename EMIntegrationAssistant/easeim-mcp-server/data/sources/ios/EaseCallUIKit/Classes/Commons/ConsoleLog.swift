...
private func consoleLog<T> (
    ...
    ChatClient.shared().log("EaseCallKit Log: \n\(timeString) \(type.rawValue) \(fileName):\(line) message:\(message) function Name:\(functionName)")
}

final class Log {
    static let logFileURL: URL = {
        let documentsDirectoryURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return documentsDirectoryURL.appendingPathComponent("CallKitLog\(CallKitVersion).log")
    }()
