...
public extension String {
    var call: CallKitWrapper<Self> {
        CallKitWrapper.init(self)
    }
    
    /// 根据下标获取字符串中的某个字符 "xxx"[2]
    internal subscript(safe index: Int) -> Character? {
        guard index >= 0, index < count else { return nil }
        return self[self.index(startIndex, offsetBy: index)]
    }
    
    internal subscript<R>(safe range: R) -> String? where R: RangeExpression, R.Bound == Int {
    ...
    @discardableResult
    internal mutating func slice(from start: Int, to end: Int) -> String {
...
public extension CallKitWrapper where Base == String {
    var numCount: Int {
        var count = 0
    ...
    func jsonToDictionary() -> Dictionary<String,Any> {
        base.data(using: .utf8)?.call.toDictionary() ?? [:]
    }
    ...
    func deleteEmoij() -> String {
        ...
        return base.call.removeMatchRegular(expression: "[^\\u0020-\\u007E\\u00A0-\\u00BE\\u2E80-\\uA4CF\\uF900-\\uFAFF\\uFE30-\\uFE4F\\uFF00-\\uFFEF\\u0080-\\u009F\\u2000-\\u201f\r\n]", with: "")
    }
    ...
    func timeStampToString(dateFormat: String?) -> Base {
        var string = base
        if(base.count >= 10){
            string = base.call.subStringTo(10)
        }
    ...
    func convertToTextImage(font: UIFont, textColor: UIColor, size: CGSize) -> UIImage? {
        ...
        let imgWidth = base.call.sizeWithText(font: font, size: size).width
