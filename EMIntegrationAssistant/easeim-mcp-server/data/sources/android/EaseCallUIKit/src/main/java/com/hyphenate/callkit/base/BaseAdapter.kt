...
/**
 * \~chinese
 * 作为RecyclerView Adapter的基类，有一个默认的空白布局
 * 你可以通过以下两种方式修改默认布局：
 * 1、在app Layout中创建一个新的callkit_layout_default_no_data.xml覆盖。
 * 2、继承BaseAdapter，重写getEmptyLayoutId()方法，返回自定义的布局。
 *
 * \~english
 ...
 * 1、Create a new callkit_layout_default_no_data.xml overlay in app Layout.
 * 2、Inheriting BaseAdapter, rewrite getEmptyLayoutId () method, return to the layout of the custom.
*/
abstract class BaseAdapter<T> :RecyclerView.Adapter<BaseAdapter.ViewHolder<T>>() {
    ...
    private val emptyLayoutId: Int
         ...
         * @return
         */
        get() = R.layout.callkit_layout_default_no_data
    ...
    protected fun getEmptyViewHolder(parent: ViewGroup): ViewHolder<T> {
        ...
        if (isHideEmptyView) {
            emptyView = LayoutInflater.from(mContext)
                .inflate(R.layout.callkit_layout_no_data_show_nothing, parent, false)
        }
     ...
     */
    fun addData(item: T) {
        synchronized(BaseAdapter::class.java) {
            if (mData == null) {
                mData = ArrayList()
            }
            mData?.add(item)
